      ...
      <div className="app-container">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '5px 0' }}>
            {user && (
              <Link to="/quiz" style={{ textDecoration: 'none' }}>
                <GenericButton label="Quiz" color="#2563eb" />
              </Link>
            )}
        ...
        <main style={{ 
          ...
          <Routes>
              ...
              user ? (
                <div style={{ width: '100%', padding: '0 20px' }}>
                  ...
                  {role === 'admin' ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
                      {/* NEW: Admin Quick Access */}
                      <div style={{ display: 'flex', gap: '15px' }}>
                        <Link to="/admin/add-questions">
                           <GenericButton label="Manage Quiz Questions" color="#10b981" />
                        </Link>
                      </div>
                      <CreateHackathon />
                      <AdminDashboard />    
                ...
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '15vh', width: '100%', padding: '0 20px', boxSizing: 'border-box' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '30px', padding: '70px 40px', width: '100%', maxWidth: '750px', background:'linear-gradient(to right,  #06B6D4, #0F172A )', backdropFilter: 'blur(20px)', borderRadius: '40px', boxShadow: '0 25px 60px -12px rgba(0,0,0,0.6)', position: 'relative', overflow: 'hidden' }}>
                    <h1 style={{ color: '#08163e', fontSize: '4.5rem', fontWeight: '900', margin: 0, letterSpacing: '-3px', textTransform: 'uppercase', position: 'relative', zIndex: 10 }}>
                      Welcome to <span style={{ color: '#0F172A ' }}>HACKNEST</span>
                    </h1>
                    <p style={{ color: '#06b4ffb7', fontSize: '1.25rem', maxWidth: '600px', margin: '0 0 20px 0', lineHeight: '1.6', fontWeight: '500', opacity: 0.9, position: 'relative', zIndex: 10 }}>
                      The unified ecosystem for the next generation of builders. <br/>
                      <span style={{ color: '#0ef1adc3', fontWeight: '700', borderBottom: '2px solid #09c8d2', paddingBottom: '3px', display: 'inline-block', marginTop: '10px' }}>Secure your identity. Start innovating.</span>
                    </p>
                    <button onClick={handleLogin} style={{ display: 'flex', alignItems: 'center', gap: '15px', padding: '16px 36px', background: 'linear-gradient(to right, #0F172A , #06B6D4)', border: 'none', borderRadius: '20px', color: '#0f172a', fontSize: '1.15rem', fontWeight: '800', cursor: 'pointer', boxShadow: '0 12px 30px rgba(0,0,0,0.4)', transition: 'all 0.3s', position: 'relative', zIndex: 10 }}>Sign in with Google</button>
                  </div>
                </div>
              )
            } />
            ...
            {/* ROUTE 3: Admin Add Questions */}
            <Route path="/admin/add-questions" element={role === 'admin' ? <AddQuestion /> : <h1 className="text-white text-center">Access Denied</h1>} />
